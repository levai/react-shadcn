...
**验证后端**：访问 http://localhost:3000/docs 查看 API 文档
...
- **开发环境**：使用 Vite 代理（已配置）
  - 前端请求 `/api/*` 自动代理到 `http://localhost:3000/api/*`
  - 无需额外配置
...
**解决**：
- 确认后端服务正在运行（访问 http://localhost:3000/docs）
- 检查后端端口是否正确（默认 3000）
...
3. **使用 Swagger**：访问 http://localhost:3000/docs 测试 API
